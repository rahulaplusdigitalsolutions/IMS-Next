...
export const serialsService = {
  getSerials: async (companyGuid) => {
    try {
      const cg = companyGuid ? `&companyGuid=${encodeURIComponent(companyGuid)}` : "";
      const res = await api.get(`/serials?_t=${new Date().getTime()}${cg}`);
      return res.data.map((item) => ({
        ...item,
        // inventorystockinserial has two unrelated `status` columns — a
        // legacy int one (raw `item.status`, meaningless here) and the real
        // availability text in `serialStatus`. Override the spread's stale
        // int with the real value so every consumer expecting a string
        // ("Available"/"Dispatched") gets one.
        status: item.serialStatus || item.status,
