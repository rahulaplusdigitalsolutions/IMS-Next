...
export async function createDispatchInline(conn, p) {
  ...
  await conn.query("UPDATE serials SET status = 'Dispatched' WHERE guid = ? AND companyGuid = ?", [p.serialId, p.companyGuid]);
  ...
  await conn.query(
    `INSERT INTO order_items
       (guid,companyGuid,orderGuid,serialNumberGuid,modelGuid,sellingPrice,warranty,contractFilename)
     VALUES (?,?,?,?,?,?,?,?)`,
    [dispatchGuid, p.companyGuid, orderId, p.serialId, modelGuid,
...
// order_items has no itemVariantId column, so the item is identified by its
// description in `remarks` (same as Draft order items already do).
export async function createNonSerializedDispatchInline(conn, p) {
  ...
  await conn.query(
    `INSERT INTO order_items
       (guid,companyGuid,orderGuid,sellingPrice,quantity,warranty,contractFilename,remarks)
     VALUES (?,?,?,?,?,?,?,?)`,
    [dispatchGuid, p.companyGuid, orderId,
