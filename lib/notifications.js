...
export async function createNotification(pool, { targetUserGuid, targetRole, title, message, type = "info", priority = "low", link = null }) {
  try {
    let usersToNotify = [];
    ...
    const values = usersToNotify.map((uid) => [uuidv4(), uid, title, message, type, priority, link]);
