...
export const POST = withErrorHandling(async (request) => {
  ...
  const [serials] = await mysqlPool.query(
    "SELECT guid, value FROM serials WHERE guid IN (?) AND godownGuid=? AND status='Available' AND isDeleted=0 AND companyGuid=?",
  ...
  try {
    ...
    try {
      await conn.query("UPDATE serials SET godownGuid=? WHERE guid IN (?) AND companyGuid=?", [destinationGodownId, serialIds, user.companyId]);
