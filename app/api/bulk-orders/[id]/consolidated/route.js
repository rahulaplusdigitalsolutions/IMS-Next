...
export const GET = withErrorHandling(async (request, { params }) => {
  ...
  const [serials] = await mysqlPool.query(
    `SELECT boi.*, s.serialNumber as serialValue, itv.variantName as modelName
     FROM bulkorderitems boi
     JOIN inventorystockinserial s ON boi.serialNumberGuid=s.guid
     LEFT JOIN inventoryitemvariant itv ON s.itemVariantId=itv.itemVariantId
     WHERE boi.orderGuid=? ORDER BY boi.addedAt ASC`,
    [id]
  );
  const [replacements] = await mysqlPool.query("SELECT r.*, oldS.serialNumber as oldSerial, newS.serialNumber as newSerial FROM replacementhistory r JOIN inventorystockinserial oldS ON r.oldSerialId=oldS.guid JOIN inventorystockinserial newS ON r.newSerialId=newS.guid WHERE r.orderGuid=? ORDER BY r.createdAt ASC", [id]);
