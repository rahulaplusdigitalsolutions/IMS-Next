...
export const GET = withErrorHandling(async (request) => {
  ...
  const params = companyGuid ? Array(10).fill(companyGuid) : [];
...
export const POST = withErrorHandling(async (request) => {
  ...
  const [check] = await mysqlPool.query("SELECT guid FROM serials WHERE value=? AND isDeleted=0 AND companyGuid = ?", [value, user.companyId]);
  if (check.length > 0) throw new ApiError(400, "Serial exists!");

  await mysqlPool.query(
    "INSERT INTO serials (guid,companyGuid,modelGuid,godownGuid,value,status,landingPrice,landingPriceReason,isDeleted,createdAt) VALUES (UUID(),?,?,?,?,?,?,?,0,NOW())",
    [user.companyId, modelId, godownGuid, value, status || "Available", landingPrice || 0, landingPriceReason || null]
  );
